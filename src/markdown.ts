...
function formatReferences(references: string[], github: string, type: "pr" | "hash"): string {
	const refs = references
		.filter((ref) => {
			if (type === "pr") return ref[0] === "#";
			return ref[0] !== "#";
		})
		.map((ref) => {
			if (!github) return ref;

			if (type === "pr") return `https://github.com/${github}/issues/${ref.slice(1)}`;
			return `[<samp>(${ref.slice(0, 5)})</samp>](https://github.com/${github}/commit/${ref})`;
		});

	const referencesString = join(refs).trim();

	if (type === "pr") return referencesString && `in ${referencesString}`;
...
function formatTitle(name: string, options: ResolvedChangelogOptions) {
	if (!options.emoji) name = name.replace(emojisRE, "");
